...
async def warewolf_function(ctx: SlashContext, joueurs: int, loups: int = 30, force: bool = False):
    ...
    if not force:
        ...
        if joueurs < 5:
            await ctx.send(f"Le nombre de joueurs doit être supérieur ou égal à 5 ! Vous avez inscrit {joueurs} joueurs.")
        elif loups > 100:
            await ctx.send(f"Le pourcentage de loup doit être inférieur ou égal à 100%! Vous avez inscrit {loups}% de loups.")
        else:
            nb_loups = round(joueurs * (loups / 100))
            await ctx.send(f"Démarrage de la partie forcé avec {joueurs} joueurs et {nb_loups} loups.")